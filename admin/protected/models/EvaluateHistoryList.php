 ...
 */
class EvaluateHistoryList extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('eva_time, modify_time, user_id, status', 'numerical', 'integerOnly'=>true),
			array('total_num, taste_num, environmental_num, service_num', 'length', 'max'=>3),
			array('account_name', 'length', 'max'=>45),
			array('desc', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('int, total_num, taste_num, environmental_num, service_num, desc, eva_time, modify_time, user_id, account_name, status', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return EvaluateHistoryList the static model class
	 */
